...
<h2><?php echo $item ? esc_html__( 'Organization 編集', 'kashiwazaki-seo-author-panel-manager' ) : esc_html__( 'Organization 追加', 'kashiwazaki-seo-author-panel-manager' ); ?></h2>
<form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=kapm&tab=organization' ) ); ?>">
    <?php wp_nonce_field( 'kapm_save_organization', 'kapm_organization_nonce' ); ?>
    <?php if ( $item ) : ?>
...
<?php if ( $item ) : $usage_type = 'organizations'; $usage_id = (int) $item['id']; include __DIR__ . '/_usage-posts.php'; endif; ?>
<p><a href="<?php echo esc_url( admin_url( 'admin.php?page=kapm&tab=organization' ) ); ?>">&larr; <?php esc_html_e( '一覧に戻る', 'kashiwazaki-seo-author-panel-manager' ); ?></a></p>
